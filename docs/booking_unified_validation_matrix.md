...
- Si impacta en reglas de comision aplicables.
- Condicion de cierre.
- Reportes afectados.
...
| Show con deuda parcial de boliche | Cachet pactado, cobrado real menor, motivo | Base operativa puede usar cobrado real; deuda boliche se mantiene | Caja sobre cobrado real | Boliche pendiente | No | Segun cobrado o segun regla definida | La comision se calcula sobre el ingreso Indyana definido por el show, segun regla vigente | Cierre parcial/observado | Deuda boliche, shows |
| Virrshi con recupero DJ set | Show normal + ajuste/recupero artista | Separa show de recupero; recupero puede repartir 70/30 | Caja show + recupero aplicado | Cuenta artista/productora | No | Si + parte recupero Indyana si aplica | Depende de reglas activas; el recupero no genera comision salvo regla explicita | Cierra show aunque cuenta recupero siga abierta si se registra | Shows, cuenta artista, recuperos |
...
| G Sony solo | Cachet, gastos, comision Facha %, split G Sony/Indyana/Fede | Descuenta Facha; reparte base entre G Sony, Indyana y tercero externo | Caja artista, Indyana y tercero si aplica | Tercero, artista, Indyana | No | Si | Generalmente excluye comision general si la comision se resolvio internamente | Cierra si G Sony, Indyana y tercero estan conciliados | Shows, terceros, comisiones |
| Candu + G Sony | Evento bruto, gastos generales, comision directa 10%, lineas Candu y G Sony | Madre calcula base; comision directa se reparte; G Sony incorpora parte; genera hijos | Caja madre + cajas hijas | Madre, hijos, tercero, Indyana | Si, Candu y G Sony | Si por cada hijo | Normalmente excluye comision general si ya hubo comision directa | Cierra madre y/o hijos segun caja y saldos | Booking compartido, shows, comisiones |
| Comision directa evento | Porcentaje/importe, beneficiarios, destino directo o incorporar a linea | Calcula salida directa o ajuste de base | Si caja VPO maneja la salida, debe aparecer | Beneficiario/tercero | No necesariamente | Puede reducir o redistribuir Indyana | No vuelve a comisionar por regla general salvo excepcion particular | Cierra cuando la salida esta registrada o marcada externa | Comisiones, caja |
...
| Gasto general de evento | Categoria, concepto, importe | Baja base general antes de lineas | Salida o gasto informado | Caja si VPO lo pago | No | Reduce Indyana indirectamente | Reduce comisiones solo por reducir el ingreso Indyana del show | Cierra si caja/gasto documentado | Gastos, rentabilidad |
| Gasto propio artista | Linea artista, categoria, concepto, importe | Baja base de esa linea | Salida si VPO lo pago | Artista/Indyana segun regla | No | Reduce Indyana de esa linea | Reduce comisiones solo por reducir el ingreso Indyana del show | Cierra con linea | Shows, artista |
| Gasto artista recuperable | Artista, proyecto, monto, recuperable, porcentaje artista/Indyana | Genera cuenta corriente recuperable | Salida inicial VPO; recuperos posteriores | Cuenta artista | No | No es show, pero afecta cuenta | No genera comision de booking salvo regla explicita | Cierra cuando se recupera o se condona | Cuenta artista, recuperos |
| Recupero aplicado en show | Linea artista, gasto recuperable origen, importe, tipo de recupero | Baja saldo recuperable; segun tipo puede afectar base antes del split o solo deuda artista | Puede entrar como recupero a caja Indyana | Saldo recuperable artista/Indyana | No | No debe mezclarse con Indyana booking salvo tipo antes del split | No genera comision de booking salvo regla explicita | Show puede cerrar si recupero queda trazado | Cuenta artista, recuperos, caja |
| Pago posterior de deuda de boliche | Show con deuda abierta, fecha de cobro, importe, metodo, comprobante | Aplica cobro contra deuda de boliche sin recalcular show | Entra caja real posterior | Baja `venue_balance_amount` o entrada operativa equivalente | No | No cambia Indyana ganado original | No cambia comisiones aplicables salvo correccion aprobada del show | Cierra deuda si queda en 0 | Cuenta booking del show, finanzas artista |
| Reintegro de artista por cobro de mas | Show con saldo a favor de Indyana, fecha, importe, metodo, comprobante | Aplica reintegro contra saldo del artista sin modificar pago original | Entra caja real posterior | Baja saldo a favor de Indyana | No | No cambia resultado del show | No cambia comisiones aplicables | Cierra cuenta si queda en 0 | Cuenta corriente artista, cuenta booking del show |
| Pago posterior al artista | Show con saldo a favor del artista, fecha, importe, metodo, comprobante | Aplica pago contra saldo a favor del artista sin modificar liquidacion original | Sale caja real posterior | Baja deuda de Indyana al artista | No | No cambia resultado del show | No cambia comisiones aplicables | Cierra cuenta si queda en 0 | Cuenta corriente artista, cuenta booking del show |
| Compensacion entre shows | Dos saldos abiertos del mismo artista/tercero, importe, nota | Aplica un saldo contra otro con trazabilidad doble | Sin caja nueva si es compensacion pura | Baja ambos saldos segun direccion | No | No cambia resultados originales | No cambia comisiones aplicables | Cierra o deja parcial segun importe | Cuenta corriente, auditoria |
| Movimiento padre - cobro total | Artista, fecha, importe total, metodo, comprobante, shows seleccionados | Crea movimiento padre y aplicaciones hijas contra shows abiertos | Entra caja real posterior | Baja saldos a favor de Indyana en varios shows | No | No cambia resultados originales | No cambia comisiones aplicables | Todos los shows aplicados quedan verdes si saldo 0 | Cuenta booking, caja, auditoria |
| Movimiento padre - cobro parcial | Artista, fecha, importe menor al total adeudado, shows seleccionados | Crea aplicaciones hasta agotar importe | Entra caja real posterior | Algunos saldos cierran y otro puede quedar parcial | No | No cambia resultados originales | No cambia comisiones aplicables | Shows saldados verdes; show parcial mantiene alerta | Cuenta booking, caja, auditoria |
| Movimiento padre - sobrante | Artista, fecha, importe mayor a los saldos seleccionados | Aplica contra shows y deja sobrante no aplicado o saldo a favor | Entra caja real posterior | Cierra shows seleccionados y registra saldo sobrante trazable | No | No inventa ingreso de show | No cambia comisiones aplicables | Shows cerrados; sobrante queda como cuenta corriente | Cuenta booking, caja, auditoria |
| Movimiento padre - compensacion multiple | Show nuevo con saldo a favor del artista, shows viejos donde artista debe a Indyana | Aplica saldo del show nuevo contra deudas viejas elegidas | Sin caja nueva | Baja saldo del show nuevo y de los shows viejos | No | No cambia resultados originales | No cambia comisiones aplicables | Cierra o deja parcial segun importe | Cuenta booking, auditoria |
...
### Regla C - Indyana ganado vs comisiones aplicables

Toda linea que genere Indyana debe responder:

- Cuanto gano Indyana.
- Que reglas de comision aplican.
- En que orden cobra cada regla.
- Si excluye comision general y si existe una excepcion particular.
- Cuanto queda neto para Indyana despues de comisiones.

Las reglas se calculan en cascada: orden 1 cobra primero, baja la base y orden 2
cobra sobre el remanente. No se permite repetir orden para el mismo artista entre
reglas activas con porcentaje mayor a cero.
