...
LAALO_PATTERN = r"laalo|laalo\s*dj|laalodj|lalo\s*dj"
...
def laalo_catalog_identifiers() -> dict[str, set[str]]:
...
def classified_rows(end_month: str) -> pl.DataFrame:
    ...
    catalog_ids = laalo_catalog_identifiers()
...
def write_report(rows: pl.DataFrame, end_month: str, *, hide_zero_amounts: bool = False) -> Path:
    ...
    output = REPORTS_DIR / f"laalo_inicio_a_{end_month}_{stamp}.xlsx"
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        ws["A1"] = f"Laalo DJ - inicio a {end_month}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Reporte personalizado Laalo DJ.")
